...
const convertToObjectId = async (numericId) => {
    if (mongoose.Types.ObjectId.isValid(numericId)) {
        return numericId;
    }
    
    // 숫자 ID인 경우 DB에서 실제 ObjectId 찾기
    const lodging = await Lodging.findOne({ id: parseInt(numericId) });
    if (lodging) {
        return lodging._id;
    }
    
    throw { status: 404, message: "숙소를 찾을 수 없습니다." };
};

// 1. 찜 추가
exports.addBookmarkService = async (userId, lodgingId) => {
    // 숫자 ID를 ObjectId로 변환
    const objectId = await convertToObjectId(lodgingId);
